...
PAGES = "https://primalconversions.github.io/listicle-swipes"

lines = [
    "# Listicle Swipe File\n\n",
    f"Full-page clones of {len(alive)} listicles" + (f" + {len(dead)} dead links" if dead else "") + " for reference when writing new listicles for clients.\n\n",
    f"**Live site:** {PAGES}/\n\n",
    "## How to use\n\n",
...
for slug, url, size in alive:
    ...
    raw = f"https://raw.githubusercontent.com/primalconversions/listicle-swipes/main/swipes/{enc}/index.html"
    lines.append(f"- `{slug}` ({mb:.1f} MB) — [view]({view}) · [raw]({raw}) · [source]({url})\n")

if dead:
    lines.append(f"\n## Dead links ({len(dead)})\n\n")
    lines.append("Source listicles no longer exist (404). Try the Wayback Machine link for each — many listicles are archived there.\n\n")
